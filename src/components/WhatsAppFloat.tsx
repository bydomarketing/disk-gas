...
const WHATSAPP_RIBEIRAO_PIRES = {
  city: 'Ribeirão Pires',
  phone: '(11) 95076-0161',
  href: 'https://wa.me/5511950760161?text=Olá! Gostaria de pedir um gás.',
  ariaLabel: 'Falar pelo WhatsApp com Ribeirão Pires'
};

const WHATSAPP_MAUA = {
  city: 'Mauá',
  phone: '(11) 93378-0522',
  href: 'https://wa.me/5511933780522?text=Olá! Gostaria de pedir um gás.',
  ariaLabel: 'Falar pelo WhatsApp com Mauá'
};
...
function WhatsAppButton({ store }: { store: { city: string, phone: string, href: string, ariaLabel: string } }) {
  return (
    <motion.a
      href={store.href}
      target="_blank"
      ...
      className="relative flex items-center gap-2.5 md:gap-3 pl-3 pr-4 md:pl-4 md:pr-6 py-2.5 md:py-3 rounded-full shadow-2xl"
    ...
    >
      ...
      <span className="relative z-10 leading-tight">
        <span className="block text-white font-black uppercase tracking-tight text-[11px] md:text-sm">{store.city}</span>
        <span className="block text-white font-bold text-xs md:text-base">{store.phone}</span>
      </span>
    </motion.a>
  );
}
    ...
    <AnimatePresence>
      {visible && (
        <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
          <WhatsAppButton store={WHATSAPP_RIBEIRAO_PIRES} />
          <WhatsAppButton store={WHATSAPP_MAUA} />
        </div>
      )}
    </AnimatePresence>
  );
